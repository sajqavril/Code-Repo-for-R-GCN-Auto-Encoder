...
class Runner(object):
	...
	def __init__(self, params):
		...
		if self.p.gpu != '-1' and torch.cuda.is_available():
			...
			self.device = torch.device('cuda:{:d}'.format(int(self.p.gpu)))
	...
	def add_model(self, model, score_func):
		...
		model = BaseModel(edge_index, edge_type, params=self.p, feature_embeddings=self.feature_embeddings, indices_2hop=indices_2hop, entity_type=self.read_data.entity_type)
	...
	def predict(self, split='valid', mode='tail_batch'):
		...
		with torch.no_grad():
			...
			for step, batch in enumerate(train_iter):
				...
				if self.p.model != 'rgcnae':
					x, r			= self.model.forward()
				else:
					x, r, gsm_score, score = self.model.forward()
	...
	def get_sparsity_constrain(self, score, lambda_=1., alpha_=10., method=12):
		if method == 12:
			idx = score.detach().abs() <= alpha_ * lambda_
			p_1 = score[idx].abs() * lambda_  - score[idx].pow(2) / (2*alpha_)
			p_0 = (~(score.detach().abs() <= alpha_ * lambda_)).to(torch.float) * (alpha_ * lambda_**2 / 2)
			p = p_1.mean() + p_0.mean()
	...
	def run_epoch(self, epoch, val_mrr = 0):
		...
		for step, batch in enumerate(train_iter):
			...
			if self.p.model != "rgcnae":
				...
				# assert self.p.use_type_feat == True
				x, r, gsm_score, score	= self.model.forward()
				
				sparsity_constrain = self.get_sparsity_constrain(score)
				_, recon_loss = self.model.get_loss(x, r, sub, rel, label, pos_neg_ent)
				loss = recon_loss + self.p.l1 * sparsity_constrain
			...
			if step % 500 == 0:
				...
				if self.p.model == 'rgcnae':
					self.logger.info('[E:{}| {}]: Train Loss:{:.5}, Train MRR:{:.5}, Val MRR:{:.5}, SP: {:.5}\t{}'.format(
						epoch, 
						step,
						np.mean(losses), 
						ave_train_mrr, 
						self.best_val_mrr, 
						(gsm_score.sum()/gsm_score.shape[0]).item(),
						self.p.name))
				else:
					self.logger.info('[E:{}| {}]: Train Loss:{:.5}, Train MRR:{:.5}, Val MRR:{:.5}\t{}'.format(
						epoch, 
						step,
						np.mean(losses), 
						ave_train_mrr, 
	...
	def fit(self):
		...
		for epoch in range(self.p.max_epochs):
			...
			if epoch % 5 == 0:
				torch.save(self.model, '{:s}_{:s}_l1_{:.4f}_lr_{:.4f}_noise{:.2f}_epoch_{:d}.pkl'.format(self.p.name, self.p.dataset, self.p.l1, self.p.lr, max(self.p.normal_noise, self.p.type_noise), epoch))
...
if __name__ == '__main__':
	...
	### add noise in training set
	parser.add_argument("-type-noise",  type=float, default=0., help="Noise that specially contradicts with type")
	parser.add_argument("-normal-noise",  type=float, default=0., help="Noise that distributed unrelated to type information")

	###### rgcnae parameters
	parser.add_argument("-use_type_feat", action='store_true',   default=False,   help="use the node type as feat input")
	parser.add_argument("-l1",  type=float, default=0.1, help="sparsity controller")
	parser.add_argument("-no_gumbel", action='store_true',   default=False,   help="use gumbel softmax or not")
	...
	args.seed = [41504, 42, 0, 1, 2][args.seed]
