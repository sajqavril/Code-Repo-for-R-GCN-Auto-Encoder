...
class KBGAT_Model(torch.nn.Module):
    def __init__(self, edge_index, edge_type, params, feature_embeddings, indices_2hop, entity_type=None):
        ...
        if feature_embeddings != None:
            ...
            if self.p.use_type_feat:
                self.init_embed		= get_param((self.num_ent_type,   self.p.init_dim))
            else:
    ...
    def forward(self, feature=None):
        ...
        if self.p.use_type_feat:
            init_embed =  self.init_embed[self.entity_type]       
            x, r = self.gat(edge_index, init_embed, self.init_rel, self.edge_type, edge_type_nhop)
            x_self = torch.matmul(init_embed, self.W_entities)
            x = x + x_self
        else:
