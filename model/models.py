...
from model.rgcn_model import RGCNAEModel
...
class BaseModel(torch.nn.Module):
	def __init__(self, edge_index, edge_type, params, feature_embeddings=None, indices_2hop=None, entity_type=None):
		...
		if self.p.model == 'compgcn':
			...
			self.model = RGCNModel(edge_index=self.edge_index, edge_type=self.edge_type, node_type=self.entity_type, params=self.p)

		elif self.p.model == 'rgcnae':
			
			self.model = RGCNAEModel(self.edge_index, self.edge_type,  self.entity_type, self.p)
			# self.init_rel = get_param(( self.p.num_rel, self.p.init_dim))
		
		elif self.p.model == 'kbgat':
			
			self.model = KBGAT_Model(self.edge_index, self.edge_type, self.p, feature_embeddings, indices_2hop, entity_type=self.entity_type)
	...
	def forward(self, feature=None):
		...
		if self.p.model == 'random':
			...
			x, r, gsm_score, score = self.model()
			return x, r, gsm_score, score
