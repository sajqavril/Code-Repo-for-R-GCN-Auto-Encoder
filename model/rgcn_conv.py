...
class RGCNConv(MessagePassing):
	...
	def forward(self, x, edge_index, edge_type, edge_norm=None):
		...
		if self.invest == 1:
			print('edge number in the model: ', edge_index.size(1), edge_type.size(0))
		...
		if self.num_bases is not None:
			...
			out = self.basic_decom_func(x, edge_index, edge_type, edge_norm=edge_norm)
		
		elif self.num_blocks is not None:
			if self.invest == 1:
				print('use low mem: ', self.low_mem)
				print('regualizer: block')
			out = self.block_decomp_func(x, edge_index, edge_type, edge_norm=edge_norm)
		...
		self.invest = 0
		return out
	def basic_decom_func(self, x, edge_index, edge_type, edge_norm):
		...
		if self.low_mem:
			...
			for i in range(self.num_relations):
				...
				h = self.propagate(aggr='mean', edge_index=tmp, x=x, edge_type=edge_type, edge_norm=edge_norm)
				out = out + (h @ weight[i])
		else:
			...
			out = self.propagate(aggr='add', edge_index=edge_index, x=x, edge_type=edge_type, norm=norm)
	...
	def block_decomp_func(self, x, edge_index, edge_type, edge_norm):
		...
		if self.low_mem:
			...
			out = self.propagate(edge_index=edge_index, x=x, edge_type=edge_type, aggr='add', norm=norm)
